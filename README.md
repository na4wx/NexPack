...
- **BBS** — read, post, and delete NexDigi BBS mail directly over RF from a remote station connecting into NexPack, driving the digipeater's real AX.25 connected-mode BBS protocol with no internet required; configured under the Chat settings tab, since RF Chat rides the same BBS session.
